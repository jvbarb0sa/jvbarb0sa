...
ROWS = [
    ("host",),
    ("kv", "Now", "Desenvolvedor @ FCS Studio"),
    ("kv", "Prev", "Engenheiro de Software"),
    ("kv", "Edu", "Ciência da Computação"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Frontend", "React, Next.js, TypeScript"),
    ("kv", "Backend", "Node.js, Python, PostgreSQL"),
    ("kv", "Cloud", "AWS, Vercel, Docker"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Desenvolvedor focado em UI/UX e performance"),
    ("bul", "Projetos incríveis criados na FCS Studio"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">joaovictor@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">joaovictor</tspan><tspan fill="{MUTED}">@</tspan>'
